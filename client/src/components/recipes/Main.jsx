...
const getRecipeJson = (savedRecipe) => (savedRecipe?.recipe_json && typeof savedRecipe.recipe_json === 'object' ? savedRecipe.recipe_json : null);

const isMissingStructuredColumns = (error) => (error?.code === 'PGRST204' || /recipe_(type|json)|cooking_method|primary_ingredient|raw_model_output/i.test(error?.message || ''));
...
export default function Main({ userId, showMessageModal, onViewSaved, onGoScan }) {
  ...
  const [recipeResponse, setRecipeResponse] = useState({ recipeType: null, raw: null });
  ...
  const fetchRecentRecipes = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.from('private_recipes').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(5);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setRecipeResponse({ recipeType: result.recipeType, raw: result.raw });
    } catch (error) {
  ...
  const handleSaveRecipe = async (recipeData, rawRecipe) => {
    ...
    const recipeToSave = recipeData || rawRecipe || recipe;
    const title = recipeData?.name?.trim() || 'Generated Recipe';
    const legacyPayload = { user_id: userId, title, ingredients: JSON.stringify(ingredients), instructions: formatRecipe(recipeToSave, title), created_at: new Date().toISOString() };
    const structuredPayload = typeof recipeData === 'object' && recipeData ? {
      ...legacyPayload,
      recipe_type: recipeResponse.recipeType || (recipeData.cookingMethod === 'drink' ? 'drink' : 'food'),
      cooking_method: recipeData.cookingMethod,
      primary_ingredient: recipeData.primaryIngredient,
      recipe_json: recipeData,
      raw_model_output: recipeResponse.raw || null,
    } : legacyPayload;
    try {
      let { error } = await supabase.from('private_recipes').insert(structuredPayload);
      if (error && structuredPayload !== legacyPayload && isMissingStructuredColumns(error)) {
        console.warn('Structured recipe columns are not available; saving legacy recipe data only.');
        ({ error } = await supabase.from('private_recipes').insert(legacyPayload));
      }
      if (error) throw error;
  ...
  return <main className="recipe-builder-shell p-4 md:p-6 w-full mx-auto pb-32">
    ...
    {!recipe && <div className="recipe-builder-card bg-white rounded-xl border border-gray-200 p-4 mb-4">
      ...
      <button onClick={handleGenerate} disabled={loading || ingredients.length < MIN_INGREDIENTS} className="w-full mt-4 py-2.5 rounded-full text-white text-sm font-medium transition-colors" style={{ backgroundColor: loading || ingredients.length < MIN_INGREDIENTS ? '#CCCCCC' : '#3B4A2F' }}>{loading ? 'Generating...' : 'Generate Recipe â†’'}</button>
    </div>}
    <button type="button" className="rounded-lg p-4 mb-4 cursor-pointer w-full text-left" style={{ backgroundColor: '#F5C842' }} onClick={onGoScan}><p className="text-[8px] uppercase text-gray-700 mb-0.5">New Â· AI Food Scanner</p><p className="text-sm font-medium text-gray-800">Snap a dish to get its recipe</p></button>
    {recentRecipes.length > 0 && <div className="mb-4"><div className="flex items-center justify-between mb-2"><h2 className="text-sm font-medium text-olive">Recent recipes</h2><button onClick={onViewSaved} className="text-xs text-gray-500">See all â†’</button></div><div className="space-y-2">{recentRecipes.map((recentRecipe) => <button key={recentRecipe.id} className="flex items-center gap-3 p-2 bg-warm rounded-lg cursor-pointer w-full text-left" onClick={() => setSelectedRecent(recentRecipe)}><FoodIcon name={getRecipeJson(recentRecipe)?.primaryIngredient || 'recipe'} size={40} /><div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800 truncate">{recentRecipe.title}</p><p className="text-xs text-gray-400">{new Date(recentRecipe.created_at).toLocaleDateString()}</p></div></button>)}</div></div>}
    {selectedRecent && <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={() => setSelectedRecent(null)}><div className="bg-white rounded-xl max-w-md w-full max-h-[80vh] overflow-auto" onClick={(event) => event.stopPropagation()}><div className="p-4 border-b border-gray-100 flex items-center justify-between"><h3 className="text-base font-medium text-olive">{selectedRecent.title}</h3><button aria-label="Close" onClick={() => setSelectedRecent(null)} className="text-gray-400 text-xl">Ã—</button></div>{getRecipeJson(selectedRecent) ? <RecipeDetail recipeData={getRecipeJson(selectedRecent)} onClose={() => setSelectedRecent(null)} readOnly /> : <pre className="p-4 text-sm text-gray-700 whitespace-pre-wrap">{selectedRecent.instructions || selectedRecent.title}</pre>}</div></div>}
